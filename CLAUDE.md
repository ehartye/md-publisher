...
- **Heading ids follow GitHub's slugger** (`lib/toc.py::slugify`): lowercase, drop everything that is not a word character/space/hyphen, then turn *each* remaining space into a hyphen without collapsing runs — so `1. Purpose & Scope` becomes `1-purpose--scope` (double hyphen where the `&` was). This is deliberate: it means a hand-authored `[Section](#anchor)` TOC resolves identically in the source `.md` on GitHub/VS Code and in the rendered PDF. Never percent-encode an anchor to match an id — fragments are compared literally, so `#a%3Ab` will not match `id="a:b"`. `find_broken_anchors` warns at build time when a link resolves to nothing, and attr_list (`## Heading {#custom-id}`) remains the escape hatch.
